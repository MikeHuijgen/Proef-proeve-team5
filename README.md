# Proef-proeve-team5
## General
This is our practice exam for software developer and game artist. We make a mobile game that is inspired by [stumble guys](https://www.stumbleguys.com/nl). Our game is about an astronaut that needs to parkour from planet to planet. The player has to dodge obstacals, jump from platform to platform and dodge the aliens that habit the planet. 

## Documentation
We document everything we do for this project inside the Github [Wiki](https://github.com/MikeHuijgen/Proef-proeve-team5/wiki). 

## Who did what
- Mike [Developer](https://github.com/MikeHuijgen)
  - State machine and base state [Documentation](https://github.com/MikeHuijgen/Proef-proeve-team5/wiki/State-machine,--base-state--&-state-intent-data)
  - Input handler [Documentation](https://github.com/MikeHuijgen/Proef-proeve-team5/wiki/Input-handler)
  - Stun state, player stun request handler and stun component [Documentation]()
  - Timer, duration timer and countdown timer [Documentation](https://github.com/MikeHuijgen/Proef-proeve-team5/wiki/Base-timer,-duration-timer,-countdown-timer)
  - Oxygen station and oxygen component [Documentation](https://github.com/MikeHuijgen/Proef-proeve-team5/wiki/Oxygen-component-and-oxygen-station)
- Gerben [Developer](https://github.com/Gerben-Prins)
  -
- Keith [Developer](https://github.com/keithko)
  -
- Jonas [Developer](https://github.com/JonasDriessen0)
  -
- Lin [Artist](https://github.com/linmorren)
  -
- Nica [Artist](https://github.com/N1cachu)
  -
- Joshua [Artist](https://github.com/drunkjoshua)
  -
- Raphael [Artist](https://github.com/Raphbama)
  -
